...
// .wrangler/tmp/pages-AncBpo/functionsWorker-0.7611518689143.mjs
...
__name(validPlayerId, "validPlayerId");
__name2(validPlayerId, "validPlayerId");
...
var TOP = 25;
var HISTORY_DAYS = 14;
var isDay = /* @__PURE__ */ __name2((v) => v !== null && /^\d{4}-\d{2}-\d{2}$/.test(v), "isDay");
...
var MAX_POINTS_PER_SECOND = 130;
var MIN_RUN_MS = 3e3;
var MAX_RUN_MS = 90 * 6e4;
var MAX_SUBMISSIONS_PER_DAY = 400;
var isInt = /* @__PURE__ */ __name2((v, min, max) => typeof v === "number" && Number.isInteger(v) && v >= min && v <= max, "isInt");
var onRequestPost = /* @__PURE__ */ __name2(async ({ request, env }) => {
  ...
  if (!isInt(score, 0, 1e7)) return bad("bad score");
  if (!isInt(section, 1, 500)) return bad("bad section");
  if (!isInt(distance, 0, 1e7)) return bad("bad distance");
  ...
  if (score > elapsedMs / 1e3 * MAX_POINTS_PER_SECOND) return bad("score impossible for the time");
  const day = dayKey();
  const now = Date.now();
  ...
  if (!existing) {
    await env.DB.prepare(
      `INSERT INTO scores (day, player_id, score, section, distance, elapsed_ms, runs, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)`
    ).bind(day, playerId, score, section, distance, elapsedMs, now, now).run();
  } else if (score > existing.score) {
    await env.DB.prepare(
      `UPDATE scores SET score = ?, section = ?, distance = ?, elapsed_ms = ?,
              runs = runs + 1, updated_at = ? WHERE day = ? AND player_id = ?`
    ).bind(score, section, distance, elapsedMs, now, day, playerId).run();
  } else {
    await env.DB.prepare(
      `UPDATE scores SET runs = runs + 1, updated_at = ? WHERE day = ? AND player_id = ?`
    ).bind(now, day, playerId).run();
  }
  const rank = await env.DB.prepare(
    `SELECT COUNT(*) + 1 AS rank FROM scores WHERE day = ? AND score > ?`
  ).bind(day, Math.max(score, existing?.score ?? 0)).first();
  return json({
    day,
...
// .wrangler/tmp/bundle-HG0URN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HG0URN/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7611518689143.js.map
